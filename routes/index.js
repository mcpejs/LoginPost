...
router.get('/list_post/:page', function (req, res) {
    countNormalPost = 'SELECT count(if(likes.likecount>=2,1,null)) AS postcount FROM posts LEFT JOIN (SELECT post_id,count(*) AS likecount FROM accountlikes GROUP BY post_id) AS likes ON posts.id=likes.post_id WHERE isNotice = 0'
    isReadNormalPosts = true
    templatePath = '../views/normalList'
    sendPostList(db, req, res, countNormalPost, isReadNormalPosts, templatePath)
})

// 추천글 조회
router.get('/list_recommendPost/:page', function (req, res) {
    countRecommendPost = 'SELECT count(if(likes.likecount>=2,1,null)) AS postcount FROM posts LEFT JOIN (SELECT post_id,count(*) AS likecount FROM accountlikes GROUP BY post_id) AS likes ON posts.id=likes.post_id WHERE isNotice = 0'
    isReadNormalPosts = false
    templatePath = '../views/recommendList'
    sendPostList(db, req, res, countRecommendPost, isReadNormalPosts, templatePath)
})
...
function sendPostList(db, req, res, getPostCountQuery, isReadNormalPosts, templatePath) {
    let postperpage = 10
    ...
    db.query(getPostCountQuery, function (err, data, fields) {
        // 마지막 페이지를 알기위한 게시물수
        ...
        let readpost
        if (isReadNormalPosts) {
            // 일반글 쿼리
            readpost = `SELECT posts.*,ifnull(likes.likecount,0) AS likeCount FROM posts LEFT JOIN (SELECT post_id,count(*) AS likecount FROM accountlikes GROUP BY post_id) AS likes ON posts.id=likes.post_id WHERE isNotice = 0 ORDER BY id DESC LIMIT ${startIndex},${postperpage}`
        } else {
            // 추천글 쿼리
            readpost = `SELECT posts.*,ifnull(likes.likecount,0) AS likeCount FROM posts LEFT JOIN (SELECT post_id,count(*) AS likecount FROM accountlikes GROUP BY post_id) AS likes ON posts.id=likes.post_id WHERE isNotice = 0 and likeCount>=2 ORDER BY id DESC LIMIT ${startIndex},${postperpage}`
        }
